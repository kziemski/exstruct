...
- [CLI-03] `--image` で PNG 出力（Excel COM 前提、`mode=libreoffice` では禁止）
- [CLI-04] `--pdf` で PDF 出力（Excel COM 前提、`mode=libreoffice` では禁止）
...
- [MODE-01] CLI `--mode` / API `extract(..., mode=)` は light/libreoffice/standard/verbose を受け付ける（デフォルト standard）
...
- [MODE-05a] `process_excel(..., mode=\"libreoffice\")` は `pdf` / `image` / `auto_page_breaks_dir` を早期に `ConfigError` で拒否する
- [MODE-05b] `mode=\"libreoffice\"` の Python runtime 検出は bundled bridge の `--probe` 成功を採用条件とし、`EXSTRUCT_LIBREOFFICE_PYTHON_PATH` の非互換指定は早期に unavailable/incompatible error として扱う
- [MODE-05c] GitHub Actions の Linux 必須 smoke job は `ubuntu-24.04` + `libreoffice` + `python3-uno` で `pytest.mark.libreoffice` smoke を skip なしで実行する
- [MODE-05d] GitHub Actions の Windows smoke job は `windows-2025` + `libreoffice-fresh` で `EXSTRUCT_LIBREOFFICE_PATH` を `soffice.exe` に固定し、`pytest.mark.libreoffice` smoke を skip なしで実行する
...
- COM: `task test-com`
- LibreOffice smoke（Linux/Windows CI 相当）: `RUN_LIBREOFFICE_SMOKE=1 pytest tests/core/test_libreoffice_smoke.py -m libreoffice -q`
