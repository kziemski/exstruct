...
- 座標系/判定ロジックのばらつき
  ...
  - 事象: 印刷範囲判定でセル →px 換算を固定値（64px/20px）で近似している箇所がある。COM がある場合は実寸を取れるが共通化されていない。
  - 対策案: 座標変換をユーティリティに集約し、COM あり/なしでの実寸/近似を明示（インターフェースに精度レベルを持たせる）。将来的に openpyxl の列幅/行高からの換算を差し替えやすくする。
...
- 関数の引数数の増大
  ...
  - 事象: engine/export 等に引数が増え続け、API 可読性が低下しつつある。
  - 対策案: 出力系オプション（include\_\* や size フラグ、print_areas_dir 等）を設定オブジェクト/ビルダーにまとめる。OutputOptions をさらに細分化（filters, destinations, formatting）すると拡張が楽になる。
